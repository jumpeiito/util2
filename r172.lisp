...
(defun main ()
  (with-excel (app :visible t :quit nil :debugger t)
	...
	(let* ((sheet (ole book :Worksheets :Item 1))
	  ...
	  (macrolet ((value-set (sym array)
		       `(progn
			  (excel::value! sheet (,sym top) (aref ,array 0))
			  (excel::value! sheet (,sym second) (aref ,array 1))
			  (excel::value! sheet (,sym bottom) (aref ,array 2)))))
	    (iter (for shibu :in (shibulist))
		  ...
		  (with-slots (r172c::shibu r172c::sc r172c::dock) counter
		    (value-set :i r172c::shibu)
		    (value-set :k r172c::sc)
		    (value-set :m r172c::dock)))))))))
